...
hook.Add(
    ...
    function()
        if KeyboardController:IsValid() then KeyboardController:Remove() end
...
do
    ...
    local function GPUSaver()
        if not systemHasFocus() or manual then
            if not state then
                if KeyboardController:IsValid() then KeyboardController:MakePopup() end
                state = true
            end
    ...
    if convar:GetBool() then hook.Add("PreRender", "GodSentToolsGPUSaver", GPUSaver) end
